...
- [ ] **Blocking I/O audit**: Final verification that every redb/tantivy access stays inside `tokio::task::spawn_blocking`. (Found violation: `ClusterCoordinator::persist_snapshot` is blocking).
...
#### 9.2 Ring Snapshot Persistence (High Priority)
**Goal**: Eliminate ring rebuild on boot for large clusters (1000+ shards).
