...
- 🚧 **Phase 11 (Workflow Hot-Path Optimizations)**: Steps 1-5 completed and verified; end-to-end search streaming and incremental write-stream ingestion remain.

### **Recommended Next Steps**
1. **True Search Streaming**: Replace buffered aggregation with end-to-end incremental NDJSON streaming across shard, router, and HTTP boundaries
2. **Incremental Write Ingestion**: Stream NDJSON writes with bounded buffering and preserved shard routing/coalescing
3. **Follow-up Validation**: Benchmark the completed Phase 11 hot-path improvements under broadcast-heavy and ingestion-heavy workloads
