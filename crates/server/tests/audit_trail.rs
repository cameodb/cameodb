...
async fn an_mcp_tool_call_records_the_tool_and_the_index() {
    let node = TestNode::start("").await;
    // `docs` has to exist for the search below to be an allowed call: an MCP search naming an
    // index the node does not have is refused, and this test is about the record rather than
    // the refusal, so the call has to succeed.
